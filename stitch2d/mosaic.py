...
CREATE_JPEG = False
USE_OPENCV = True
OPENCV_MATCHER = 'brute-force'
OPENCV_SCALAR = 0.5
OPENCV_THRESHOLD = 0.7
...
class Mosaic(object):
    ...
    def __init__(self, path, skipped=None, jpeg=CREATE_JPEG,
                 opencv=USE_OPENCV, **kwargs):
        # Process basic class parameters
    ...
    def set_mosaic_parameters(self):
        ...
        self.rows = mandolin(self.tiles, self.num_cols)
        self.num_rows = len(self.rows)
        self.mag = prompt(' Magnification:', '^\d+(\.\d)?$')
        #self.mag = 200
        ...
        if self.opencv:
            cprint('  Equalize hist:'
                   ' {}'.format(self.cv_params['equalize_histogram']))
        ...
        if prompt('Do these parameters look good?', yes_no):
            ...
            keys = sorted(self.coordinates.keys(), key=lambda s:
                            'x'.join(['0'*(4-len(n))+n
...
def mosey(path=None, skipped=None, jpeg=CREATE_JPEG,
          opencv=USE_OPENCV, **kwargs):
